/* HIZAYA PWA – MQTT.js (WSS) – no-build
   - Login mock
   - MQTT via mqtt.min.js (browser) → plus de Paho
...
(function(){
  ...
  const LS_KEYS = {
    user: "hz_user",
    masters: "hz_masters",
    broker: "hz_broker", // {url}
  };

  // Broker WSS public (OK avec ton ESP sur 1883/TCP en parallèle)
  ...
  function ensureMQTTjs(){
    if (window.mqtt && typeof window.mqtt.connect === "function") return true;
    alert("Bibliothèque MQTT.js introuvable. Vérifie la balise <script> mqtt.min.js dans index.html.");
    return false;
  }
  ...
  function connectMQTT(){
    ...
    try{
      ...
      alert("Erreur création client MQTT (mqtt.min.js non chargé ?)");
      console.error(err);
      return;
    }
    ...
    state.client.on("error", (err) => {
      log("MQTT erreur:", err?.message || err);
    });
  ...
  function disconnectMQTT(){
    if(state.client){
      try{ state.client.end(true); }catch{}
    }
    state.connected=false;
    state.client=null;
    log("MQTT déconnecté");
  ...
  function unsubscribeMaster(mid){
    ...
    const st  = `hizaya/${mid}/state`;
    try{
      state.client.unsubscribe([evt, st]);
  ...
  function renderMasters(){
    ...
    state.masters.forEach((m)=>{
      ...
      // rename master inline (autosave 500ms)
  ...
  document.addEventListener("DOMContentLoaded", ()=>{
    // Login
    const savedUser = load(LS_KEYS.user);
    if(savedUser){ state.user=savedUser; showMain(); } else { showLogin(); }

    $("#btnLogin").onclick = ()=>{
      const email=$("#email").value.trim(); if(!email) return alert("Entre un email");
      state.user={email}; save(LS_KEYS.user,state.user); showMain();
    };
