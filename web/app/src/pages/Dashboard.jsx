...
// constantes
const LIVE_TTL_MS = 8_000;      // master "en ligne" si vu < 8s
const DEFAULT_IO_PIN = 26;      // pin par défaut pour IO sur le SLAVE
  ...
  return (
    <div className="actionBarBlock">
      ...
      <div className="actionBarWrapper">
        <div className={
          "actionBarFill " + (phase==="queue" ? "queueAnim" :
                              phase==="send"  ? "sendAnim"  :
                              isAck           ? "ackedFill" : "")
        }/>
      </div>
...
function MasterCard({
  device, slaves, onMasterRename, onMasterDelete, onSendMasterCmd,
...
export default function Dashboard(){
  ...
  // realtime channels
  const chDevices=useRef(null), chNodes=useRef(null), chCmds=useRef(null), chGroups=useRef(null);
      ...
      .on("postgres_changes", {event:"*", schema:"public", table:"commands"}, (payload)=>{
        ...
        if(row?.target_mac){
          if(row.status==="acked"){
            ...
            setTimeout(()=>setSlavePhases(old=>({...old, [row.target_mac]:"idle"})),2000);
          }
        }
        if(row?.status==="acked"){
  ...
  useEffect(()=>{
    ...
    sb.auth.getSession().then(({data})=>{
      ...
      if(sess?.user){
        setUser(sess.user);
        setAuthReady(true);
        (async()=>{ await fullReload(); attachRealtime(); })();
      }else{
    ...
    const { data: sub } = sb.auth.onAuthStateChange(async (event, session)=>{
      if(!mounted) return;
      if(event==="SIGNED_IN"){ setUser(session?.user||null); await fullReload(); attachRealtime(); }
      else if(event==="SIGNED_OUT"){ setUser(null); setDevices([]); setNodesByMaster({}); setGroupsData([]); setSlavePhases({}); cleanupRealtime(); }
      else if(event==="TOKEN_REFRESHED" || event==="USER_UPDATED"){ setUser(session?.user||null); }
      setAuthReady(true);
    });
    return ()=>{ mounted=false; sub?.subscription?.unsubscribe(); cleanupRealtime(); };
  },[]);
  ...
  async function deleteMaster(id){
    ...
    if(!r.ok){ const txt=await r.text(); addLog("❌ Suppression : "+txt); } else { addLog(`MASTER supprimé : ${id}`); }
    await fullReload();
  }
  ...
  async function renameGroup(id){
    ...
    if(error){ window.alert("Erreur rename group: "+error.message); } else { addLog(`Groupe ${id} renommé en ${newName}`); await refetchGroupsOnly(); }
  }

  async function deleteGroup(id){
    if(!window.confirm("Supprimer ce groupe ?")) return;
    const { error: e1 } = await sb.from("group_members").delete().eq("group_id",id); if(e1){ window.alert("Erreur suppr membres groupe: "+e1.message); return; }
    const { error: e2 } = await sb.from("groups").delete().eq("id",id); if(e2){ window.alert("Erreur suppr groupe: "+e2.message); return; }
  ...
  async function askAddMaster(){
    ...
    const ttlSec = Math.floor((new Date(expires_at).getTime() - Date.now())/1000);
  ...
  function renameAccountLabel(){
    const newLabel=window.prompt("Nom du compte ?", accountName || (user?.email||"")); if(!newLabel) return;
  ...
  const currentSlaveInfo = useMemo(()=>{
    ...
    const list = nodesByMaster[slaveInfoOpen.masterId] || [];
    return list.find(s=>s.mac===slaveInfoOpen.mac) || null;
          ...
          <div className="groupListWrap">
            {groupsData.map(g=>(
              <GroupCard
                ...
                onOpenMembersEdit={openGroupMembersModal}
                onOpenOnList={openGroupOnListModal}
                onGroupCmd={sendGroupCmd}
              />
            ))}
          </div>
        )}
