      ...
      <div className="masterTopRow">
        <div className="masterTitleLeft">
          ...
          <div className="masterMeta smallText">
            ...
            <span className="kv"><span className="k">Dernier contact :</span>{" "}
              <span className="v">{device.last_seen ? fmtTS(device.last_seen) : "jamais"}</span></span>
          </div>
        </div>
...
export default function Dashboard() {
  ...
  // Si chargé et pas connecté → redirection vers /login
  useEffect(() => {
    if (authReady && !user) {
      navigate("/login", { replace: true });
    }
  ...
  async function handleLogout() {
    ...
    // la redirection est gérée aussi dans onAuthStateChange, mais on force :
    navigate("/login", { replace: true });
  }
  ...
  if (!authReady) {
    return (
      <div style={{ color: "#fff", padding: "2rem" }}>Chargement…</div>
    );
  }
  if (!user) {
    // Le redirect effect gère déjà, on met un garde-fou
    return null;
  }
